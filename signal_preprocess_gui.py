...
class SignalPreprocessApp(tk.Tk):
    ...
    def draw_selection_span(self, ax, n: int, interval_s: float) -> None:
        try:
            ...
            if x1 > x0:
                ax.axvspan(x0, x1, alpha=0.12, label="截取范围")
        except Exception:
            return
    ...
    def _plot_displacement(self, ax, t: np.ndarray, raw: np.ndarray, proc: Optional[np.ndarray], max_points: int) -> None:
        ...
        raw_label = "角度位移/原始" if self.current_source_kind == "angle" else (f"所选轴 {self.axis_var.get()}" if self.current_source_kind == "axis" else "原始")
        ...
        title = "角度位移波形" if self.current_source_kind == "angle" else ("加速度轴波形" if self.current_source_kind == "axis" else "位移/信号波形")
        ax.set_title(title)
        ax.set_xlabel("时间 (s)")
        ax.set_ylabel("角度 (°)" if self.current_source_kind == "angle" else "数值")
    ...
    def _plot_velocity(self, ax, raw: np.ndarray, proc: Optional[np.ndarray], interval_s: float, max_points: int) -> None:
        ...
        ax.set_xlabel("时间 (s)")
        ax.set_ylabel("°/s" if self.current_source_kind == "angle" else "单位/s")
        ax.grid(True, alpha=0.3)
        ax.legend(loc="best")
